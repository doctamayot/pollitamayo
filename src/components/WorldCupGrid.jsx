   ...
   const matchesByDate = useMemo(() => {
    ...
    return (
        <div className="max-w-5xl mx-auto pb-24 animate-fade-in px-2 sm:px-0">
             <div 
    className="fixed left-0 top-16 md:top-0 z-[100] w-full h-12 sm:h-14 bg-background/95 backdrop-blur-xl flex items-center shadow-2xl border-b border-border/50 rounded-b-2xl overflow-hidden"
...
>
    <div className="w-full flex-1">
        <NewsTicker />
            ...
            <div className="sm:mt-10 mt-7 bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl sm:rounded-[2rem] p-3 sm:p-10 mb-6 sm:mb-8 text-center border border-border shadow-xl relative overflow-hidden flex flex-row items-center justify-center gap-3 sm:gap-6">
            ...
            {isAdmin && (
                <div className="mb-8 bg-purple-900/20 border border-purple-500/30 p-4 sm:p-5 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-[0_0_15px_rgba(168,85,247,0.05)] animate-fade-in">
                    ...
                    <div className="flex items-center gap-2 w-full sm:w-auto">
                        ...
                        <button 
                            onClick={handleManualSyncByTeamNames} 
                            className="bg-blue-500/20 text-blue-400 px-3 py-2 sm:py-2.5 rounded-xl font-black hover:bg-blue-500/40 transition-colors text-xs flex items-center gap-1.5 uppercase tracking-widest ml-2" 
                            title="Extraer goles de la API usando nombres"
                        >
                            <span>📡</span> <span className="hidden sm:inline">Sync por Equipos</span>
                        </button>
                    </div>
                </div>
            )}
            ...
            <div className="space-y-6 sm:space-y-10">
                {sortedMatchesOfDay.map(match => {
                    ...
                    // Solo mostramos números en pantalla si de verdad existen goles guardados por el Admin
                    const hasO = (rH !== undefined && rH !== '' && rA !== undefined && rA !== '');
