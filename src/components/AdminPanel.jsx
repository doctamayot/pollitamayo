...
const AdminPanel = ({ quiniela }) => {
        ...
        if (!currentQuinielaData.isClosed) {
            ...
            try {
                ...
                scores.forEach(score => {
                    ...
                    // --- ▼▼▼ LÓGICA PARA LA NUEVA INSIGNIA "PÓLVORA MOJADA" ▼▼▼ ---
            ...
            // --- LÓGICA COMPLETA Y CORRECTA PARA RE-ABRIR ---
    ...
    return (
        <div className="bg-slate-700/50 p-4 rounded-lg mb-6 flex flex-col lg:flex-row items-center justify-between space-y-4 lg:space-y-0">
            <h3 className="text-white font-bold text-sm sm:text-base">Panel de Admin: <span className="text-amber-300">{quiniela.name}</span></h3>
            <div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-4">
                <div className="flex items-center">
                    <span className={`mr-3 text-sm font-medium ${quiniela.isActive ? 'text-green-400' : 'text-slate-300'}`}>Activa</span>
                ...
                <div className="flex flex-col items-center">
                    <button
                        ...
                        className={`font-bold py-2 px-4 rounded-md text-sm transition ${quiniela.isClosed ? 'bg-yellow-500 hover:bg-yellow-600 text-black' : 'bg-red-600 hover:bg-red-700 text-white'} disabled:bg-slate-500 disabled:cursor-not-allowed`}
                    >
                        {isUpdating ? 'Procesando...' : (quiniela.isClosed ? 'Re-abrir Quiniela' : 'Cerrar y Puntuar')}
                    </button>
                    {!allResultsFilled && !quiniela.isClosed && (<span className="text-xs text-slate-400 mt-1">Llenar resultados para cerrar</span>)}
                </div>
            </div>
        </div>
    );
